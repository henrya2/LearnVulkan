...
- **Viewport flip**: `vk::Viewport.height` is negative and `y` starts at `extent.height`. This preserves the Y-up orientation from NDC to screen space, so CCW winding in NDC remains CCW in framebuffer coordinates. Therefore `front_face` is `COUNTER_CLOCKWISE`, matching the mesh winding convention (CCW from outside).
- **glTF coordinate conversion**: glTF is RH Y-up; the project is LH Y-up. Conversion: negate Z in positions, normals, tangent xyz; flip tangent.w; convert transform matrices via `diag(1,1,-1,1) * M * diag(1,1,-1,1)`. After Z-negation, glTF CCW winding becomes CW in LH world space; however, with the negative viewport height preserving winding orientation through the full transform pipeline, the PBR pipeline still uses `COUNTER_CLOCKWISE` because the RH-to-LH conversion also reverses the sense of front/back in the transform.
- **Descriptor strategy**: Two descriptor sets — set 0 per-frame global UBO (view, proj, camera, light) + material buffer + env map; set 1 per-material textures (5 bindings). No descriptor indexing required.
