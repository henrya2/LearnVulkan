...
- **Texture upload:** staging buffer -> device-local `vk::Image` via two `cmd_pipeline_barrier` transitions (`UNDEFINED -> TRANSFER_DST_OPTIMAL -> SHADER_READ_ONLY_OPTIMAL`). Format is `R8G8B8A8_SRGB`, sampler uses `REPEAT` addressing for floor tiling.
